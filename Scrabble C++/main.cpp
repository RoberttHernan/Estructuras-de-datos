...
// Libreria de cabecera unica para parseo de JSON (nlohmann/json)
...
// Llena la lista circular doble de un jugador con fichas tomadas de la cola
...
// Verifica si una letra especifica esta disponible en la mano del jugador
bool buscarLetraEnListaJugador(listaEnlazadaDobleCircular<Letra> *lista, string letra);
string ToUpperCase(string st1);
// Verifica si las fichas colocadas en el turno forman al menos una palabra valida del diccionario
bool match(listaSimple<Casillas> *lista);
void cambiarFichas(listaEnlazadaDobleCircular<Letra> *lista_letras_jugador);
// Retorna el nodo de la lista del jugador que corresponde a la letra buscada
...
// Retorna el multiplicador de la casilla (1=Normal, 2=Doble, 3=Triple)
int BuscarDoble_Triple(int x, int y);
void catchError();
void ReportePuntajePorJugador();
void terminarPartida();
// Recorre el ABB de jugadores y los inserta ordenados en la lista de puntajes totales
...
int dimension;           // dimension N del tablero N x N
int puntajeJugadorUno = 0;
int puntajeJugadorDos = 0;
int turno = 1;           // 1 = turno del jugador uno, 2 = turno del jugador dos

// Casillas especiales (Doble/Triple) cargadas desde el JSON
listaSimple<Casillas> *listaCasillas = new listaSimple<Casillas>();
// Diccionario de palabras validas (lista doblemente enlazada circular de strings)
listaEnlazadaDobleCircular<string> *listaPalabras = new listaEnlazadaDobleCircular<string>();
// Bolsa de fichas disponibles (cola FIFO)
Cola<Letra> *cola = new Cola<Letra>();
// ABB que almacena todos los jugadores registrados ordenados por nombre
ABB *arbol = new ABB();
Jugador jugador_uno;
// Mano de fichas del jugador uno (lista doblemente enlazada circular)
listaEnlazadaDobleCircular<Letra> *lista_jugador_uno = new listaEnlazadaDobleCircular<Letra>();
Jugador jugador_dos;
// Mano de fichas del jugador dos (lista doblemente enlazada circular)
listaEnlazadaDobleCircular<Letra> *lista_jugador_dos = new listaEnlazadaDobleCircular<Letra>();
// Tablero de juego representado como matriz dispersa
Matriz<string> *tablero = new Matriz<string>();
// Fichas colocadas en el turno actual, pendientes de validacion
listaSimple<Casillas> *listaCasillasTemp = new listaSimple<Casillas>();
// Lista ordenada de jugadores con su mayor puntaje para el reporte de ranking
listaSimple<Jugador> *listaPuntajeTotal = new listaSimple<Jugador>();
...
/*
 * Carga la configuracion del juego desde un archivo JSON.
 * Lee la dimension del tablero, las coordenadas de casillas dobles y triples,
 * y las palabras del diccionario. Las palabras se convierten a mayusculas
 * antes de insertarse en listaPalabras.
 */
void config(string ruta)
{
    try
...
/*
 * Llena la cola global con fichas en orden aleatorio siguiendo la distribucion
 * oficial del Scrabble en espanol. Cada letra tiene una cantidad y un valor
 * en puntos determinados por las reglas del juego:
 *   A=12(1pt), E=12(1pt), O=9(1pt), I=6(1pt), S=6(1pt), N=5(1pt), R=5(1pt),
 *   U=5(1pt), T=5(1pt), D=5(2pt), L=4(1pt), C=4(3pt), G=2(2pt), B=2(3pt),
 *   M=2(3pt), P=2(3pt), H=2(4pt), F=1(4pt), V=1(4pt), Y=1(4pt), Q=1(5pt),
 *   J=1(8pt), X=1(8pt), Z=1(10pt).
 *
 * Se usa una cola auxiliar de numeros para marcar que letras ya se agotaron
...
// Busca una letra en la mano del jugador. Retorna true si la encuentra.
...
/*
 * Valida si las fichas colocadas en el turno forman al menos una palabra valida.
 * Para cada casilla colocada en la lista temporal, busca la palabra formada en
 * su columna (buscarPalabraMatriz_x) y en su fila (buscarPalabraMatriz_y).
 * Si ambas busquedas fallan para una casilla, se incrementa el contador de fallos.
 * El turno es valido unicamente si el contador de fallos es cero.
 */
bool match(listaSimple<Casillas> *lista)
{
...
// Convierte una cadena a mayusculas. Se usa para normalizar las palabras
// del diccionario al momento de cargarlas desde el JSON.
string ToUpperCase(string st1)
{
...
// Retorna el nodo de la lista del jugador que contiene la letra buscada, o nullptr si no existe.
NodoDCLL<Letra> *buscarLetraEnListaJugador_nodo(listaEnlazadaDobleCircular<Letra> *lista, string letra)
{
    NodoDCLL<Letra> *temp = lista->getHead();
    int contador = 1;
...
/*
 * Calcula el puntaje obtenido por las fichas colocadas en la columna x o fila y.
 * Recorre primero la columna; si la palabra formada en esa columna esta en el
 * diccionario, suma el puntaje de cada letra multiplicado por el bonus de su casilla.
 * En caso contrario hace lo mismo con la fila y.
 */
int getPuntuacionTotal(int x, int y)
{
...
// Reinicia el estado de la partida: limpia el tablero, las manos de los jugadores,
// los puntajes del turno y vuelve a llenar la bolsa de fichas.
void terminarPartida()
{
